...
public class MatchCards {
    ...
    for (String cardName : cardList) {
        ...
        ImageIcon cardImageIcon;
        if (imgURL == null) {
            // create a simple placeholder image for missing assets
            System.out.println("Missing image: " + cardName + " -> using placeholder");
            java.awt.image.BufferedImage placeholder = new java.awt.image.BufferedImage(cardWidth, cardHeight, java.awt.image.BufferedImage.TYPE_INT_RGB);
            java.awt.Graphics2D g = placeholder.createGraphics();
            try {
                g.setColor(new Color(245, 245, 245));
                g.fillRect(0, 0, cardWidth, cardHeight);
                g.setColor(new Color(200, 200, 200));
                g.drawRect(1, 1, cardWidth-3, cardHeight-3);
                g.setColor(new Color(80, 80, 80));
                g.setFont(new Font("Arial", Font.PLAIN, 14));
                java.awt.FontMetrics fm = g.getFontMetrics();
                String text = cardName;
                int tx = (cardWidth - fm.stringWidth(text)) / 2;
                int ty = (cardHeight - fm.getHeight()) / 2 + fm.getAscent();
                g.drawString(text, Math.max(4, tx), Math.max(fm.getAscent()+4, ty));
            } finally {
                g.dispose();
            }
            cardImageIcon = new ImageIcon(placeholder);
        } else {
            Image cardImg = new ImageIcon(imgURL).getImage();
            cardImageIcon = new ImageIcon(
